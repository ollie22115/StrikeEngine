...
namespace Strike {
	...
	public:
		...
		void loadStaticRenderable(const Renderable& renderable) override;
		void drawDynamicRenderable(const Renderable& renderable) override;
		void flush(std::shared_ptr<Window>& window, const glm::mat4& viewMatrix, const glm::mat4& projectionMatrix) override;
	...
	private:
		...
		struct GLMesh {
			//RenderDrawMode mode; TODO!!!
			ResourcePointer<Material> materialPtr;
			int32_t vertexCount;
			int32_t offset;

			GLMesh() = default;
			inline GLMesh(const ResourcePointer<Material>& materialPtr, int32_t vertexCount, int32_t offset) : 
				materialPtr(materialPtr), vertexCount(vertexCount), offset(offset) {}
